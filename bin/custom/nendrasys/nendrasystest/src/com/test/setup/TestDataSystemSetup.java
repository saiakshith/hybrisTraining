...
import com.test.constants.NendrasysTestConstants;
import com.test.orders.AcceleratorTestOrderData;
 ...
 */
@SystemSetup(extension = NendrasysTestConstants.EXTENSIONNAME)
public class TestDataSystemSetup extends AbstractSystemSetup
{
	...
	public void createProjectData(final SystemSetupContext context)
	{
		if (getBooleanSystemSetupParameter(context, CREATE_TEST_DATA))
		{
			...
			importImpexFile(context, "/nendrasystest/import/qa-test-users.impex");

			LOG.info("Creating Reviews...");
			importImpexFile(context, "/nendrasystest/import/reviews.impex");

			LOG.info("Creating Test Payment Subscriptions...");
			getAcceleratorTestOrderData().createPaymentInfos();

			LOG.info("Creating CS Tickets...");
			importImpexFile(context, "/nendrasystest/import/cstickets.impex");
			...
			importImpexFile(context, "/nendrasystest/import/csquotes.impex");
		}
	}
}
